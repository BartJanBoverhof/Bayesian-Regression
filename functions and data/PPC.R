...
ppc <- function(x, t, data){
  ...
  data <- data %>% mutate(order1 = sort(mean(x[,1]) + mean(x[,2])*data[,iv[1]] + mean(x[,3])*data[,iv[2]] + mean(x[,4])*data[,iv[3]] + mean(x[,5])*data[,iv[4]])) %>% 
    mutate(order2 = sort(mean(x[,7]) + mean(x[,8])*data[,iv[1]] + mean(x[,9])*data[,iv[2]] + mean(x[,10])*data[,iv[3]] + mean(x[,11])*data[,iv[4]])) #Create ordering for both chain 1 and 2
  ...
  for (i in 1:t){ #Loop over the amount of replications t
    ...
    resid.obs <- data.chain1[,dv] - x[i,1] + x[i,2]*data.chain1[,iv[1]] + x[i,3]*data.chain1[,iv[2]] + x[i,4]*data.chain1[,iv[3]] + x[i,5]*data.chain1[,iv[4]] #Calculating residuals
    ...
    mu <- x[i,1] + x[i,2]*data.chain1[,iv[1]] + x[i,3]*data.chain1[,iv[2]] + x[i,4]*data.chain1[,iv[3]] + x[i,5]*data.chain1[,iv[4]]
    sim <- rnorm(nrow(data),mu,sqrt(x[i,6]))
    
    resid.sim <- sim - x[i,1] + x[i,2]*data.chain1[,iv[1]] + x[i,3]*data.chain1[,iv[2]] + x[i,4]*data.chain1[,iv[3]] + x[i,5]*data.chain1[,iv[4]] #Calculating residuals
    ...
    #CHAIN 2
    #TEST STAT OBSERVED DATA
    resid.obs.2 <- data[,dv] - x[i,7] + x[i,8]*data.chain2[,iv[1]] + x[i,9]*data.chain2[,iv[2]] + x[i,10]*data.chain2[,iv[3]] + x[i,11]*data.chain2[,iv[4]] #Calculating residuals
    ...
    #TEST STAT SIMULATED DATA
    mu.2 <- x[i,7] + x[i,8]*data.chain2[,iv[1]] + x[i,9]*data.chain2[,iv[2]] + x[i,10]*data.chain2[,iv[3]] + x[i,11]*data.chain2[,iv[4]]
    sim.2 <- rnorm(nrow(data),mu.2,sqrt(x[i,12]))
    
    resid.sim.2 <- sim.2 - x[i,7] + x[i,8]*data.chain2[,iv[1]] + x[i,9]*data.chain2[,iv[2]] + x[i,10]*data.chain2[,iv[3]] + x[i,11]*data.chain2[,iv[4]] #Calculating residuals
